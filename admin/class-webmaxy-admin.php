...
class WebMaxy_Admin {
	...
	public function enqueue_styles() {
		 ...
		 * defined in WebMaxy_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The WebMaxy_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */

		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/webmaxy-admin.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/webmaxy-admin.js', array( 'jquery' ), $this->version, false );

	}
	public function addPluginAdminMenu() {
		//add_menu_page( $page_title, $menu_title, $capability, $menu_slug, $function, $icon_url, $position );
		add_menu_page(  $this->plugin_name, 'WebMaxy', 'administrator', $this->plugin_name, array( $this, 'displayPluginAdminSettings' ), 'dashicons-chart-area', 26 );
		
		//add_submenu_page( '$parent_slug, $page_title, $menu_title, $capability, $menu_slug, $function );
		//add_submenu_page( $this->plugin_name, 'WebMaxy Settings', 'Settings', 'administrator', $this->plugin_name.'-settings', array( $this, 'displayPluginAdminSettings' ));
	}
	...
	public function registerAndBuildFields() {
		...
		add_settings_section(
			...
			'WebMaxy_general_section', 
			// Title to be displayed on the administration page
			'',  
			// Callback used to render the description of the section
				array( $this, 'WebMaxy_display_general_account' ),    
			// Page on which to add this section of options
			'WebMaxy_general_settings'                   
		);
		unset($args);
		$args = array (
							'type'      => 'input',
		...
		add_settings_field(
			'webmaxy_site_id',
			'WebMaxy ID',
			array( $this, 'WebMaxy_render_settings_field' ),
			'WebMaxy_general_settings',
			'WebMaxy_general_section',
			$args
		);
	...
	public function WebMaxy_display_general_account() {
		 echo '<p>Visit <a href="https://www.webmaxy.co">https://www.webmaxy.co</a> to get WebMaxy ID</p>';
	} 
	public function WebMaxy_render_settings_field($args) {
			/* EXAMPLE INPUT
								'type'      => 'input',
								'subtype'   => '',
